...
def chunk_text(text: str,
               chunk_size: int = 1750,
               chunk_overlap: int = 210) -> List[str]:
...
async def fetch_texts_batch(sql: str = query_sql, limit: int = 1, tax_type: str | None = None, is_batch: bool = False, id_limit_down: int = 0 | None, id_limit_up: int = 10000000 | None) -> List[Dict[str, Any]]:
    params: Tuple[Any, ...] = ()
    if is_batch:
            ...
            for chunk_id, chunk_text in enumerate(chunks, start=1):
                for q_id, q in enumerate(questions, start=1):
                    for model_id, model_name in enumerate(LLM_MODELS, start=1):
                        # Limit parallelism
                        async def one_call(chunk_id=chunk_id, chunk_text=chunk_text, q=q, model_name=model_name):
                            async with self.llm_sem:
                                answer, latency_ms = await self.llm_answer(model_name, chunk_text, q)
                            # 6a) Save QA result
                            is_excluded = False
                            qa_id = await insert_qa_result(conn, text_id, chunk_id, q_id, q, model_id, model_name, chunk_text, answer, is_excluded, latency_ms)
                            # 4b/6b) Vectorize answer + save embedding
                            vec = await self.embed_answer(answer)
                            await insert_answer_embedding(conn, qa_id, text_id, tax_type, q_id, show_in_chat, is_excluded, vec)
                            return 1
