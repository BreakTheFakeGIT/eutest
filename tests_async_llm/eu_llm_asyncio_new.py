...
async def insert_qa_results(conn: psycopg.AsyncConnection,
    ...
    sql = sql.SQL("""
    INSERT INTO {schema_name}.{tabele_name} (tax_type, interp_id, chunk_cnt, chunk_id, chunk_text, question_id, question, answer, latency_ms, experiment)
    ...
    try: 
        async with conn.cursor() as cur:
            await cur.execute(sql, (tax_type, interp_id, chunk_cnt, chunk_id, chunk_text, question_id, question, answer, latency_ms, experiment))
...
# -----------------------------
async def chunk_ans_embedding1024(conn: psycopg.AsyncConnection,
                            schema_name: str = SCHEMA_NAME,
                            tabele_name: str = 'chunk_ans_embedding1024',
                            chunk_ans_id: int,
                            qa_results_id: int,
                            tax_type: str,
                            interp_id: int,
                            ...
                            chunk_ans_latency_ms: Dict[str, int],
                       ) -> None:
    sql = sql.SQL("""
    INSERT INTO {schema_name}.{tabele_name} (chunk_ans_id, qa_results_id, tax_type, interp_id, chunk_id, intepretation, experiment, embedding, chunk_ans_latency_ms)
    VALUES ();
    """).format(schema_name=sql.Identifier(schema_name),tabele_name=sql.Identifier(tabele_name))
    ...
    try:
        async with conn.cursor() as cur:
            await cur.execute(sql, (chunk_ans_id, qa_results_id, tax_type, interp_id, chunk_id, intepretation, experiment, embedding, chunk_ans_latency_ms))
    except Exception as e:
        logger.error(f"Error inserting chunk_ans_embedding1024 for interp_id={interp_id}, tax_type={tax_type}, chunk_id={chunk_id}: {e}")
        raise


async def model_ans_embedding1024(conn: psycopg.AsyncConnection,
                            schema_name: str = SCHEMA_NAME,
                            tabele_name: str = 'model_ans_embedding1024',
                            qa_results_id: int,
                            tax_type: str,
                            interp_id: int,
                            intepretation: Dict[str, Any],
                            ...
                            experiment: Dict[str, Any],
                            embedding: List[float],
                            model_ans_latency_ms: Dict[str, int],
                          ) -> None:

    sql = sql.SQL("""
    INSERT INTO {schema_name}.{tabele_name} (qa_results_id, tax_type, interp_id, intepretation, question_id, experiment, embedding, model_ans_latency_ms)
    VALUES ();
    """).format(schema_name=sql.Identifier(schema_name),tabele_name=sql.Identifier(tabele_name))
    ...
    try:
        async with conn.cursor() as cur:
            await cur.execute(sql, (qa_results_id, tax_type, interp_id, intepretation, question_id, experiment, embedding, model_ans_latency_ms))
    except Exception as e:
...
class TaxRAGPipeline:
    ...
    async def process_context(self, context_row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with await psycopg.AsyncConnection.connect(PG_CONN_STR) as conn:
            tasks = []
            for question_id, q in enumerate(questions, start=1):
                for experiment_key in chunks_experiment.keys():
                    ...
                    for chunk_id, chunk_text in enumerate(chunk, start=1):
                        ...
                        async def one_call(tax_type=tax_type,
                            ...
                            print(f"interp_id={interp_id}, tax_type={tax_type}, q='{q}'")
                            # print(latency_ms)
                            # print(answer)
                            # print(experiment)
                            ...
                            if question_id == 1: # First question - chunk text
...
#         raise
                           ...
                           # await insert_answer_embedding768(conn,
                            ...
                            #     return 0  # Skip empty answers
                                # print(f"interp_id={interp_id}, tax_type={tax_type}, q='{q}', model={LLM_MODELS[0]}")
                                # #Save QA result
...
#                                                             )
#                             print(f"Inserted QA result id={qa_id} for interp_id={interp_id}, tax_type={tax_type}, model={model_name}, chunk_id={chunk_id}.")
