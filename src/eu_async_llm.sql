...
-- PROD: MAIN TABLE!!! QA results per chunk/question/model
...
-------------------------------------------------------------------------
-- PROD: OPTIONAL (without REFERENCES qa_results(id) ON DELETE CASCADE)
-- CHANGE sa to vat, cit, pit ?????
-- GOAL is SPLITED BY tax_type AND INSERT INTO answer_embeddings_qa
-------------------------------------------------------------------------
-- DROP TABLE answer_embeddings_sa
-- TRUNCATE TABLE answer_embeddings_sa
...
-- OPTION
-------------------------------------------------------------------------
--DELETE FROM qa_results --(CASCADE works)
...
SELECT * FROM qa_results ORDER BY id DESC
SELECT * FROM answer_embeddings_qa ORDER BY id DESC
SELECT * FROM stat_results ORDER BY id DESC
SELECT * FROM answer_embeddings_sa ORDER BY id DESC 
...
WHERE qa_results_id = ANY(SELECT MIN(qa_results_id) FROM t0 AS tb WHERE ta.text_id = tb.text_id)
...
-- TEST: MAIN TABLE!!! QA results per chunk/question/model
...
-- TEST: Embeddings of answers for retrieval/RAG
...
-------------------------------------------------------------------------
-- TEST OPTIONAL (without REFERENCES qa_results(id) ON DELETE CASCADE)
-- CHANGE sa to vat, cit, pit ?????
-- GOAL is SPLITED BY tax_type AND INSERT INTO answer_embeddings_qa_test
-------------------------------------------------------------------------
-- DROP TABLE answer_embeddings_sa_test
-- TRUNCATE TABLE answer_embeddings_sa_test
