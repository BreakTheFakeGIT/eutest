...
import pandas as pd
import pickle
from src.extract.text_eu_json import TextEuJson
...
logger = logger_utils.setup_logger(name='extract_eu_json')
logger.info(f"EU_DICT path: {EU_DICT}")

# Function to read eu_dict CSV file and return two dictionaries
def get_eu_dict(path: str) -> tuple[dict, dict]:
    ...
    data = pd.read_csv(path_file,
        ...
        engine='python',
        encoding='utf-8',
        usecols=['id','opis','wartosc','slownikid','typ_podatku'],
        dtype={'id': int, 'opis': str,'wartosc': str, 'slownikid': int, 'typ_podatku': str}
        )
    ...
    return eu_dict_opis, eu_dict_wartosc, eu_dict_typ_podatku 

# Function to join list of strings with dictionary values
...
def single_json_line(json_line):
    ...
    eu_dict_opis, eu_dict_wartosc, eu_dict_typ_podatku = get_eu_dict(path=EU_DICT)

    if 'przepisy' in dict2 or 'zagadnienia' in dict2 or 'slowa_kluczowe' in dict2:
        ...
        fields = ['przepisy', 'zagadnienia', 'slowa_kluczowe']
        for field in fields:
            dict2[f'{field}_opis_eu'] = join_list_and_dict(
                string_list=dict2.get(field, []),
                value_dict=eu_dict_opis
            )
            dict2[f'{field}_wartosc_eu'] = join_list_and_dict(
                string_list=dict2.get(field, []),
                value_dict=eu_dict_wartosc
            )
        ...
        dict2['kod_eu'] = None
        dict2['opis_eu'] = None
        dict2['wartosc_eu'] = None
        logger.info(f"Keys 'przepisy', 'zagadnienia', or 'slowa_kluczowe' not found in the dictionary. {dict2.items()}")
