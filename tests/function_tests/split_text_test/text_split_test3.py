...
# To use this script, you need to have langchain and psycopg installed:
# pip install langchain psycopg
...
def experiment_text_splitting(
    ...
    **splitter_kwargs,
    ...
    This function iterates through specified chunk sizes and overlaps, applies a chosen
    LangChain text splitter, and then filters the resulting chunks based on exclusion
    criteria (specific phrases, whole words, or regular expressions).

    Args:
        text (str): The input text to be split.
        chunk_sizes (List[int]): A list of chunk sizes to experiment with.
        chunk_overlaps (List[int]): A list of chunk overlaps to experiment with.
        splitter_class (Type[TextSplitter]): The LangChain TextSplitter class to use.
            Defaults to RecursiveCharacterTextSplitter. Other options include
            CharacterTextSplitter.
        exclude_phrase (Optional[str]): A phrase to exclude chunks by. If a chunk
                                        contains this phrase, it will be discarded.
        exclude_word (Optional[str]): A word to exclude chunks by. If a chunk
                                      contains this whole word, it will be discarded.
        exclude_regex (Optional[str]): A regular expression. If a chunk matches this
                                       regex, it will be discarded.
        **splitter_kwargs: Additional keyword arguments to pass to the splitter's
                           constructor (e.g., `separator` for CharacterTextSplitter
                           or `separators` for RecursiveCharacterTextSplitter).

    Returns:
        Dict[str, List[str]]: A dictionary where keys are descriptive strings of the
        experiment parameters (e.g., "size_100_overlap_20") and values are the
        lists of resulting text chunks after filtering.
    """
    results = {}
...
def write_results_to_postgres(
    ...
    This function connects to a PostgreSQL database, creates a table if it
    doesn't exist, and inserts the chunks from the results dictionary.

    Args:
        results (Dict[str, List[str]]): The dictionary of experiment results from
                                        the experiment_text_splitting function.
        db_params (Dict[str, str]): A dictionary containing database connection
                                    parameters (e.g., dbname, user, password, host, port).
    """
    create_table_sql = """
...
if __name__ == "__main__":
    ...
    write_results_to_postgres(results_no_exclusion, db_params = POSTGRES_DSN)
